...
local RandomSpawnableGrenades = nil
function ents.MORBUS.GetSpawnableGrenades(random)

   if not SpawnableGrenades then
      local tbl = {}
      local rtbl = {}
      for k,v in pairs(weapons.GetList()) do
         if v and v.AutoSpawnable and WEPS.IsGrenade(v) then
            table.insert(tbl, v)
            if !v.NeverRandom then
               table.insert(rtbl,v)
            end
      ...
      RandomSpawnableGrenades = rtbl
   end

   if random then
      return RandomSpawnableGrenades
   else
