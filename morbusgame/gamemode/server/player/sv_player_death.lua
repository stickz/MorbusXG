...
function GM:PlayerDeath( victim, weapon, killer )
    ...
    victim:SetNWBool("IsSpawning", true)
	timer.Simple(4, function() victim:SetNWBool("IsSpawning", false) end)	
...
function Death_Swarm_Infect( ply, weapon, killer )
    ...
    if killer:GetBaseSanity() > 700 then
        killer:SetSwarmPoints( killer:GetSwarmPoints() + 1 )
        PlayerMsg( killer, "You have earned one Swarm Point!", false )

    else
        PlayerMsg( killer, "You gained no swarm point! (low sanity)", false )
    end
end
