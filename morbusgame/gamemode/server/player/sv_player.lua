...
function GM:PlayerInitialSpawn(ply)
   ...
   ply.WantsSpec = false
   ply.LastWentSpec = CurTime()
   ply.LastCommittedSucide = CurTime()
	ply.Human_Model = nil
   ply.MaxHealth = 100
   ...
   --sswarm spawn protection
   ply.SpawnProtected = false
   ply.CanUseResponse = true
 ...
 --  ply:SetDeathFX( 0 )
   ...
   if table.KeyFromValue(SteamIDlist, playerSteamID) then
      ply:SetNWInt("Donator",2)
	   
      if (playerSteamID == "STEAM_0:0:20749231") then --PLease leave [Remscar Original Dev]
         ply.ForceName = "Zachary Nawar" --This is me the Dev, this just gives me my name and Red scoreboard name color
         ply.ForceGender = 2
   
      elseif (playerSteamID == "STEAM_0:1:7305295") then -- [Sonoran Warrior - Did a lot of maps]
         ply.ForceName = "Tits McGee"
         ply.ForceGender = 2

      elseif (playerSteamID == "STEAM_0:0:10342150") then -- [Demonkush - lots of stuff]
         ply.ForceName = "Mr. Kush"
         ply.ForceGender = 2

      elseif (playerSteamID == "STEAM_0:0:22105310") then -- [CvG admin who helped]
         ply.ForceName = "Misty Temple"
         ply.ForceGender = 1

      elseif (playerSteamID == "STEAM_0:1:10651334") then
         ply.ForceName = "Fur Ball"
         ply.ForceGender = 2

      elseif (playerSteamID == "STEAM_0:0:30900996") then
         ply.ForceName = "Talroc Robinson"
         ply.ForceGender = 2

      elseif (playerSteamID == "STEAM_0:1:40238734") then
         ply.ForceName = "Luke Skywalker"
         ply.ForceGender = 2
 
      elseif (playerSteamID == "STEAM_0:0:5305311") then
...
function GM:PlayerSpawn(ply)
   ...
   if (GetRoundState() == ROUND_WAIT) then
	...
	if ply:IsSwarm() then
		--ply:PrintMessage(HUD_PRINTTALK, "[xG] Spawn protection activated!")
...
function GM:CanPlayerSuicide(ply)
	if !ply:IsAdmin() then 	
		if CAN_SUICIDE == false then
			ply:PrintMessage(HUD_PRINTTALK, "[xG] Please wait 30s after round start before using a suicide command!")
			return false
		end
		
		if ply.LastCommittedSucide > CurTime() then
			ply:PrintMessage(HUD_PRINTTALK, "[xG] Please wait 30s after before committing sucide again!")
			return false
		end
	end
	
	ply.LastCommittedSucide = CurTime() + 30
	return true
end
...
function GM:OnPlayerHitGround(ply, in_water, on_floater, speed)
   ...
   if math.floor(damage) > 0 then
      ...
      if ply:GetNWBool("alienform",false) == true then
         ...
         if ply.Upgrades[UPGRADE.LEGS] then
            dmg:ScaleDamage(0)
         end
...
--function GM:CanPlayerSuicide(ply)	
--	ply:PrintMessage(HUD_PRINTTALK, "[xG] Please wait eight seconds for your suicide request to complete!")
--	timer.Simple(8, function() if IsValid(ply) && ply:IsGame() ply:Kill() end)	
--	return false
--end