...
function GM:PlayerCanPickupWeapon(ply, wep)
   ...
   local isPlayerSwarm = ply:IsSwarm() 
   
   if isPlayerSwarm and weaponClass != "weapon_mor_swarm" then
   	return false
   elseif !isPlayerSwarm and weaponClass == "weapon_mor_crowbar" then
      return true
   elseif ValidEntity(awep) and awep:GetClass() == "weapon_mor_brood" then
   	return false
   elseif !isPlayerSwarm and weaponClass == "weapon_glowstick_special" then
      return true
   elseif weaponClass == "weapon_mor_swarm" and !isPlayerSwarm then
   	return false
   elseif weaponClass == "weapon_mor_brood" and !ply:IsBrood() then
      return false
   ...
   if ValidEntity(ply) and ValidEntity(wep) then
	 ...
	 if not IsValid(wep) then return end
      ...
      timer.Simple(0.1,function() 
					   if IsValid(wep) then wep:PhysWake() end  
	                   end)
