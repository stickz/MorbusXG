...
function GM:PlayerTakeDamage(ent, infl, att, amount, dmginfo)
   ...
   local inflClass = infl:GetClass()

   if att:IsPlayer() then
   
      if (!infl:IsWeapon()) && dmginfo:IsBulletDamage() then
         infl = att:GetActiveWeapon()
      end

      // Infection timer
      if att:IsSwarm() && !ent:IsAlien() then
		 if att.SpawnProtected == true then
      ...
      if att:IsBrood() then
         if att:GetNWBool("alienform",false) && (inflClass == "weapon_mor_brood") then
         ...
         // Brood Aliens do reduced gun and grenades damage
         if !(att:GetNWBool("alienform",false)) && !(inflClass == "weapon_mor_brood") then
            dmginfo:ScaleDamage(0.25)		
		 elseif inflClass == "env_explosion" then
			dmginfo:ScaleDamage(0.6)
         end	  
	  end

      // Brood Aliens take extra damage when in human form *DISABLED in 1.5.7*
      if ent:IsBrood() && !ent:GetNWBool("alienform",false) && inflClass != "weapon_mor_crowbar" then
   ...
   if (math.floor(dmginfo:GetDamage()) > 0) then
	  ...
	  if ent:IsSwarm() == true then
         ent:EmitSound(table.Random(Sounds.Swarm.Pain),200,100)
      elseif ent:GetNWBool("alienform",false) == true then
   ...
   if ent != att and ValidEntity(att) and att:IsPlayer() and GetRoundState() == ROUND_ACTIVE and math.floor(dmginfo:GetDamage()) > 0 then
      local txt = ""
      if inflClass == "env_explosion" then
         infl = "Grenade"
...
function GM:ScalePlayerDamage(ply, hitgroup, dmginfo, real)
   ...
   if hitgroup == HITGROUP_HEAD then
      ...
      if table.KeyFromValue(tableHitGroup, hitgroup) then   
         dmginfo:ScaleDamage(0.85)
      end
   ...
   if atkr:IsPlayer() then
      ...
      if ply:IsAlien() && (atkr:IsSwarm() || atkr:GetNWBool("alienform")) then dmginfo:SetDamage(0) end
   end
