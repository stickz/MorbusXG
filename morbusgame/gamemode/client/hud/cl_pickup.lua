...
function GM:HUDDrawPickupHistory()
   ...
   for k, v in pairs( self.PickupHistory ) do
      ...
      if v.time < CurTime() then
         ...
         draw.SimpleText( vName, vFont, v.x+2+v.height+8, v.y - (v.height/2)+2, Color( 0, 0, 0, alpha*0.75 ) )

         draw.SimpleText( vName, vFont, v.x+v.height+8, v.y - (v.height/2), Color( 255, 255, 255, alpha ) )

         if vAmount then
            draw.SimpleText( vAmount, vFont, v.x+self.PickupHistoryWide+2, v.y - (v.height/2)+2, Color( 0, 0, 0, alpha*0.75 ), TEXT_ALIGN_RIGHT )
            draw.SimpleText( vAmount, vFont, v.x+self.PickupHistoryWide, v.y - (v.height/2), Color( 255, 255, 255, alpha ), TEXT_ALIGN_RIGHT )
         end
