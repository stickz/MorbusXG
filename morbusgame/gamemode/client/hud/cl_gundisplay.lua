...
function DrawAmmoScreen()
	...
	if ply:Alive() then
		...
		if weapon and weapon:IsValid() and vmodel and vmodel:IsValid() then
			...
			--add as variable to speed up hud display
			local weaponClass = weapon:GetClass()
			if (weapon:Clip1() > 0 && weaponClass != "weapon_glowstick" && weaponClass != "weapon_glowstick_special") then
				...
				if GunHud.enabled then		
					...
					if ironsight || weaponClass == "weapon_mor_medkit" then

						local ang = ply:EyeAngles()
						local pos = ply:EyePos() + ang:Forward()*10 + ang:Up()*-3

						if weaponClass != "weapon_mor_medkit" then
							ang = ironsight.Ang
							pos = ironsight.Pos
						end
...
function DrawAmmoScreenTexture()
	...
	if ply:Alive() then
		...
		if weapon and weapon:IsValid() and vmodel and vmodel:IsValid() then
			...
			--add as variable to speed up hud display
			local weaponClass = weapon:GetClass()
			if (ammo > 0 && weaponClass != "weapon_glowstick" && weaponClass != "weapon_glowstick_special") then
				...
				cam.Start2D()
					...
					surface.SetDrawColor(math.floor(ScreenColor.r*bgscale),math.floor(ScreenColor.g*bgscale),math.floor(ScreenColor.b*bgscale),64)
					surface.SetTexture(NoiseTexture)
					surface.DrawTexturedRect(0, 15, TEX_WIDTH/2.3, TEX_HEIGHT/2)
					...
					surface.SetFont("PrAmmoFont")
					local PrimaryAmmoW, PrimaryAmmoH = surface.GetTextSize(PrimaryAmmo)
					surface.SetTextPos((TEX_WIDTH/5)-(PrimaryAmmoW/2),TEX_HEIGHT/3.5)
					surface.DrawText(PrimaryAmmo)
