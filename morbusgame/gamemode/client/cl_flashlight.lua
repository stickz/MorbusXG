...
function LIGHT.Draw(ply)
	if !GetGlobalBool("mutator_nightmare",false) then
		...
		local width = ScrW()/2-wide/2
		local borderWidth = width + border
		
		local height = ScrH()-by
		local borderHeight = height + border
		
		local borderWide = wide-border*2		
		
	  	draw.RoundedBox(2, width, height, wide, tall, Color(0,0,0,230))
	  	draw.RoundedBox(2, borderWidth, borderHeight, borderWide, tall-border*2, Color(120,120,120,230))
		
 	 	local ratio = math.Clamp(ply.Battery/LIGHT_BATTERY,0,1)
 	 	draw.RoundedBox(0, borderWidth, borderHeight, borderWide*ratio, tall-border*2, Color(255,205,0,230))
 	 	draw.RoundedBox(0, borderWidth, borderHeight, borderWide*ratio, tall-border*4, Color(255,180,0,230))
	end
end
