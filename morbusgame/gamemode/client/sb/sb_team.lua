...
local bgcolor = Color(20,20,20, 50)
function PANEL:Paint()   
   ...
   -- Darkened background
   draw.RoundedBox(0, 0, 0, wide, self:GetTall(), bgcolor)
   ...
   draw.RoundedBox(0, 0, 0, self:GetWide(), 20, self.color)
   ...
   for i, row in ipairs(self.rows_sorted) do
      if (i % 2) != 0 and not extend then
         surface.SetDrawColor(60,60,60,80)
         surface.DrawRect(0, y, wide, row:GetTall())
      end
   ...
   surface.DrawRect(wide - 125, 0, 100, self:GetTall())
end
...
function PANEL:PerformLayout()
   ...
   local wide = self:GetWide()   
   self:SetSize(wide, 30 + self.rowcount + self.rowcount * SB_ROW_HEIGHT)
   ...
   for k, v in ipairs(self.rows_sorted) do
	  ...
	  if expand and (k % 2) == 0 then
		v:SetPos(wide / 2 + 1, y - (tall + 1) )
	  else
		v:SetPos(0, y)
		y = y + tall + 1
	  end
