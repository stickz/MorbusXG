...
function SWEP:SecondaryAttack()
	...
	if 	self.Weapon:GetNWBool("Reloading") or 
		self:GetNextSecondaryFire() > CurTime() or
		self.Owner:KeyDown(IN_USE) or 
		self.Owner:KeyDown(IN_SPEED)
	then return end
	
	local sights = not self:GetIronsights()
	
	self.Scoped = sights	
	self:SetIronsights(sights, self.Owner)
...
---------------------------------------------------------*/
--function SWEP:Think()
	--self.Weapon:SetNWBool( "IsLaserOn", !self.Weapon:GetNWBool("Reloading"))
	--self:IronSight()
--end
...
function SWEP:Reload()
	...
	self.Weapon:DefaultReload(self.Silenced and ACT_VM_RELOAD_SILENCED or ACT_VM_RELOAD) 
	...
	timer.Simple(waitdammit + .1, 
		function() 
			...
			self.Weapon:SetNWBool("Reloading", false)
			--self.Weapon:GetClass() == self.Gun &&
			if self.Scoped == true then
...
function SWEP:SetIronsights(b)
	...
	--this is really lame, but is better than think code
	for i = 0.3, 0.9 do
