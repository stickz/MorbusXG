...
function SWEP:Deploy()
	...
	--local nextPrim = "next primary "..(self.Weapon:GetNextPrimaryFire() - CurTime())
	--PrintMessage(HUD_PRINTTALK, nextPrim)
	...
	--local afterPrim = "after primary "..(self.Weapon:GetNextPrimaryFire() - CurTime())
	--PrintMessage(HUD_PRINTTALK, afterPrim)
	...
	if (self.Weapon:Clip1() < self.Primary.ClipSize and self.Owner:GetAmmoCount(self.Primary.Ammo) > 0) then
		...
		timer.Simple(0.3, function()
			...
			if self.Weapon then
				...
				self.Weapon:SetNWInt("reloadtimer", CurTime() + 0.5)
				self.Weapon:SetNextPrimaryFire(CurTime() + 0.5)
				self.Weapon:SetNextSecondaryFire(CurTime() + 0.5)
			end
		end)
